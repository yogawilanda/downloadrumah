...
<div x-data="{ searchOpen: false }" @click.outside="searchOpen = false" class="relative">
    ...
    @if ($variant === 'compact')
        ...
        <form wire:submit.prevent="submitSearch"
            class="bg-white p-2 sm:p-2.5 rounded-md border border-gray-200 shadow-sm flex flex-col md:flex-row items-center gap-2 relative">
            <div class="relative w-full md:flex-1">
                <input type="text" wire:model.live.debounce.300ms="search" @focus="searchOpen = true"
                ...
                <x-layouts.home.search-suggestions :suggestions="$suggestions" :search="$search" :popularCities="$popularCities" />
            </div>
        ...
        <div class="relative bg-sky-600 rounded-md p-5 md:p-8 text-white shadow-md overflow-visible">
            <div class="relative z-10 space-y-4">
                <div>
                    ...
                    <p class="text-xs md:text-sm text-blue-100">Lihat-lihat dulu berdasarkan lokasi, kebutuhan, dan
                        budget.</p>
                </div>

                <form wire:submit.prevent="submitSearch"
                    class="bg-white p-2 rounded-md text-gray-800 shadow-lg grid grid-cols-1 md:grid-cols-12 gap-2 items-center relative">
                    <!-- Search Input & Autocomplete Suggestions -->
                    <div class="md:col-span-5 relative">
                        <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-wider px-2">Lokasi /
                            Properti</label>
                        <input type="text" wire:model.live.debounce.300ms="search" @focus="searchOpen = true"
                            placeholder="Cari lokasi, nama properti..."
                    ...
                    <!-- Dropdown Kota -->
                    <div
                        class="md:col-span-3 border-t md:border-t-0 md:border-l border-gray-100 pt-2 md:pt-0 pl-0 md:pl-2">
                        <label
                            class="block text-[10px] font-bold text-gray-400 uppercase tracking-wider px-2">Kota</label>
                        <select wire:model.live="city"
                            class="w-full md:w-auto text-xs sm:text-sm border-0 bg-transparent text-gray-700 focus:ring-0 cursor-pointer">
                            <option value="">Semua Kota</option>
                            @foreach ($cities as $c)
                                <option value="{{ \Illuminate\Support\Str::slug($c->name) }}">{{ $c->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <!-- Dropdown Status -->
                    <div
                        class="md:col-span-2 border-t md:border-t-0 md:border-l border-gray-100 pt-2 md:pt-0 pl-0 md:pl-2">
                        <label
                            class="block text-[10px] font-bold text-gray-400 uppercase tracking-wider px-2">Status</label>
                        <select wire:model.live="transaction_type"
                            class="w-full px-1 py-1 text-xs font-semibold text-gray-800 border-0 focus:ring-0 bg-transparent cursor-pointer focus:outline-none">
