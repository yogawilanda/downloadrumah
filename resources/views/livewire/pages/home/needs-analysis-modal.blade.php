    ...
    <div class="w-full max-w-md bg-white rounded-2xl shadow-xl overflow-hidden p-6 space-y-6">
            ...
            <div class="space-y-4">
                ...
                <div class="space-y-2">
                    @foreach(['kos' => 'Sewa Kos Bulanan/Tahunan', 'sewa_rumah' => 'Sewa Rumah / Kontrakan', 'beli_rumah' => 'Beli Rumah (Nabung DP KPR)'] as $key => $label)
                        <label class="flex items-center gap-3 p-3 border rounded-md cursor-pointer hover:bg-indigo-50/50 transition has-[:checked]:border-indigo-600 has-[:checked]:bg-indigo-50">
        ...
        @if($currentStep === 2)
            <div class="space-y-4">
                ...
                <input type="number" wire:model="budgetTarget" step="500000" class="w-full p-3 border rounded-md text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
        ...
        @if($currentStep === 3)
            <div class="space-y-4">
                ...
                <input type="number" wire:model="monthlySaving" step="100000" class="w-full p-3 border rounded-md text-sm focus:ring-2 focus:ring-indigo-500 outline-none">
            </div>
        @endif
        ...
        <div class="flex items-center justify-between pt-4 border-t">
            ...
            @if($currentStep < 3)
                <button wire:click="nextStep" class="px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs rounded-md shadow transition">Lanjut →</button>
            @else
                <button wire:click="submitAnalysis" class="px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-bold text-xs rounded-md shadow transition">Lihat Rekomendasi</button>
            @endif
        </div>
