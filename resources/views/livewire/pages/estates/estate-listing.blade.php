            ...
            <div class="bg-white rounded-2xl border border-gray-100 shadow-sm p-3 space-y-3">
                <div class="flex gap-3 items-center">
                    <div class="w-20 h-20 rounded-xl overflow-hidden bg-gray-100 flex-shrink-0 relative">
                        @php
                            $img = $estate->primaryImage;
                            $imgUrl = null;

                            if ($img) {
                                if (!empty($img->url)) {
                                    $imgUrl = $img->url;
                                } elseif (!empty($img->file_path)) {
                                    $imgUrl = \Illuminate\Support\Facades\Storage::url($img->file_path);
                                }
                            }
                        @endphp
                ...
                <div class="pt-2 border-t border-gray-50">
                    @if ($tab === 'my_listings')
                        <div class="grid grid-cols-2 gap-2">
                            ...
                            <button wire:click="deleteEstate({{ $estate->id }})"
                                class="py-2 bg-red-50 text-red-600 text-xs font-bold text-center rounded-xl">Hapus</button>
                        </div>
                    @else
                        <!-- Action khusus Co-Broke -->
                        <a href="{{ route('estates.show', $estate->slug) }}"
                            class="block w-full py-2 bg-emerald-50 text-emerald-600 text-xs font-bold text-center rounded-xl">
