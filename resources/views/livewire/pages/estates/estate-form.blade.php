<div class="w-full pb-28 pt-4 px-4 max-w-lg mx-auto">
    ...
    <form wire:submit="save" class="space-y-4">
        ...
        <div class="bg-white p-4 rounded-xl border border-gray-100 shadow-sm space-y-3">
            <h2 class="text-xs font-semibold text-gray-800 uppercase tracking-wider border-b border-gray-100 pb-2">
                Informasi Utama</h2>
        ...
        <div class="bg-white p-4 rounded-xl border border-gray-100 shadow-sm space-y-3">
            <h2 class="text-xs font-semibold text-gray-800 uppercase tracking-wider border-b border-gray-100 pb-2">
                Fasilitas & Legalitas</h2>

            <div class="grid grid-cols-2 gap-3">
                <label class="flex items-center space-x-2 text-xs text-gray-700">
                    <input type="checkbox" wire:model="attributes_list.garage"
                        class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                    <span>Garasi</span>
                </label>

                <label class="flex items-center space-x-2 text-xs text-gray-700">
                    <input type="checkbox" wire:model="attributes_list.swimming_pool"
                        class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                    <span>Kolam Renang</span>
                </label>

                <label class="flex items-center space-x-2 text-xs text-gray-700">
                    <input type="checkbox" wire:model="attributes_list.garden"
                        class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                    <span>Taman</span>
                </label>

                <label class="flex items-center space-x-2 text-xs text-gray-700">
                    <input type="checkbox" wire:model="attributes_list.pam"
                        class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                    <span>Air PAM</span>
                </label>
            </div>
            ...
            <div class="pt-2">
                ...
                <select wire:model="attributes_list.certificate"
                    class="w-full rounded-lg border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 text-sm">
        ...
        <!-- Foto Properti -->
        <div class="bg-white p-4 rounded-xl border border-gray-100 shadow-sm space-y-3">
            <h2 class="text-xs font-semibold text-gray-800 uppercase tracking-wider border-b border-gray-100 pb-2">Foto
                Properti</h2>
                ...
                <div class="grid grid-cols-3 gap-2">
                    @foreach($existingPhotos as $photo)
                        @php
                            ...
                            // Bersihkan prefix 'public/' atau slash di awal jika ada
                            $cleanPath = ltrim(str_replace('public/', '', $filePath), '/');

                            // Tembak langsung ke route /media/ yang baru dibuat
                            $photoUrl = url('media/' . $cleanPath);
                        @endphp
                        <div class="relative aspect-square rounded-lg overflow-hidden border border-gray-200">
                            <img src="{{ $photoUrl }}" class="w-full h-full object-cover" alt="Foto Properti">
                            <button type="button" wire:click="deleteExistingPhoto({{ $photoId }})"
                                class="absolute top-1 right-1 bg-red-600 text-white rounded-full p-1 opacity-90 text-xs shadow-sm">
            ...
            <label
                class="flex flex-col items-center justify-center w-full h-24 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50 active:bg-gray-100">
        ...
        <div class="pt-2 flex gap-3">
            <a href="{{ route('dashboard') }}"
                class="w-1/3 text-center py-3 rounded-xl border border-gray-300 bg-white text-gray-700 text-xs font-semibold flex items-center justify-center active:bg-gray-50 shadow-sm">
                Batal
            </a>
