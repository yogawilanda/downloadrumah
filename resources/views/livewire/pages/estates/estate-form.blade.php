<div class="w-full pb-28 pt-4 px-4 max-w-lg mx-auto">
    ...
    <div class="mb-6 px-2">
        <div class="relative flex items-center justify-between">
            ...
            <div class="absolute left-0 top-1/2 transform -translate-y-1/2 h-0.5 bg-blue-400 -z-10 transition-all duration-300"
                style="width: {{ (($currentStep - 1) / 3) * 100 }}%;"></div>
            @foreach ([1 => 'Info Umum', 2 => 'Detail Properti', 3 => 'Info Tambahan', 4 => 'Konfirmasi'] as $step => $label)
                <div class="flex flex-col items-center">
                    <button type="button" wire:click="setStep({{ $step }})"
                        class="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition {{ $currentStep >= $step ? 'bg-blue-400 text-gray-900 ring-4 ring-blue-100' : 'bg-gray-200 text-gray-500' }}">{{ $step }}</button>
                    <span
        ...
        <div class="pt-4 flex gap-3">
            ...
            @if ($currentStep < 4)
                <button type="button" wire:click="nextStep"
                    class="{{ $currentStep > 1 ? 'w-2/3' : 'w-full' }} py-3 rounded-xl bg-blue-600 text-white text-xs font-bold shadow-md active:bg-blue-500 transition">
                    Selanjutnya
                </button>
            @else
                <button type="submit" wire:loading.attr="disabled" wire:target="save"
                    class="{{ $currentStep > 1 ? 'w-2/3' : 'w-full' }} py-3 rounded-xl bg-blue-400 text-gray-900 text-xs font-bold shadow-md active:bg-blue-500 transition disabled:opacity-50 disabled:cursor-not-allowed">
