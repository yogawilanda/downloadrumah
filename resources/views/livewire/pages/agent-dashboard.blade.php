...
<div class="w-full pb-32 pt-4 px-4 max-w-md mx-auto space-y-6">
    ...
    <div class="bg-white p-4 rounded-md border border-slate-100 shadow-sm flex items-center justify-between">
        <div>
        ...
        <a href="{{ route('estates.create') }}" wire:navigate
            class="px-3.5 py-2.5 bg-blue-600 text-white text-xs font-bold rounded-md active:scale-95 transition flex items-center gap-1 shadow-sm shadow-blue-200">
            <span>+ Properti</span>
        </a>
    </div>
    ...
    <div class="grid grid-cols-3 gap-2.5">
        <div class="bg-white p-3.5 rounded-md border border-slate-100 shadow-sm flex flex-col justify-between">
    ...
    <div class="space-y-2">
        ...
        <div class="grid grid-cols-2 gap-2.5">
            <a href="{{ route('dashboard.estates') }}" wire:navigate
                class="p-3.5 bg-white rounded-md border border-slate-100 shadow-sm hover:border-blue-200 transition flex flex-col justify-between">
                <p class="text-sm font-bold text-slate-800">Kelola Listing</p>
                <p class="text-[11px] text-slate-400 mt-0.5">Atur & Publikasikan</p>
            </a>

            <div
                class="p-3.5 bg-white rounded-md border border-slate-100 shadow-sm opacity-80 flex flex-col justify-between relative overflow-hidden">
            ...
            <div
                class="p-3.5 bg-white rounded-md border border-slate-100 shadow-sm opacity-80 flex flex-col justify-between">
                <div class="flex items-center justify-between">
            ...
            @forelse($estates->take(5) as $estate)
                <a href="{{ route('estates.show', $estate->slug) }}" wire:navigate
                    ...
                    <div class="flex items-center gap-3.5 min-w-0 flex-1">
                        <div
                            class="w-14 h-14 rounded-md bg-slate-100 flex-shrink-0 overflow-hidden relative border border-slate-100">
