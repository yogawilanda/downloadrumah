...
<div class="min-h-screen bg-slate-50/50 pb-28 lg:pb-12 relative font-sans antialiased">
    ...
    <div class="max-w-6xl mx-auto px-0 sm:px-4 md:px-6 lg:px-8 pt-0 sm:pt-6">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
            ...
            <div class="lg:col-span-8 bg-white sm:rounded-xl sm:border sm:border-slate-200/80 sm:shadow-sm overflow-hidden">
                ...
                <div class="aspect-video w-full bg-slate-100 relative overflow-hidden">
                    ...
                    <span class="absolute top-3 left-3 bg-slate-900/80 backdrop-blur-sm text-white text-[10px] font-extrabold uppercase px-2.5 py-1 rounded-md tracking-wider">
                ...
                <div class="p-4 sm:p-6 lg:p-8 bg-white space-y-6">
                    <div class="space-y-3">
                        ...
                        <div>
                            <h1 class="text-2xl sm:text-3xl font-black text-blue-600 tracking-tight mb-1">{{ $estate->formatted_price }}</h1>
                    ...
                    <div class="grid grid-cols-4 gap-2 sm:gap-4 py-3.5 px-3 bg-slate-50 rounded-lg text-center border border-slate-100">
                        @foreach ([['Kamar', $estate->bedroom, 'KT'], ['Mandi', $estate->bathroom, 'KM'], ['Luas Bgn', $estate->building_size, 'm²'], ['Luas Tnh', $estate->land_size, 'm²']] as $index => [$label, $value, $unit])
                            <div class="space-y-0.5 {{ $index > 0 ? 'border-l border-slate-200/60' : '' }}">
                                <span class="block text-[10px] sm:text-xs font-semibold text-slate-400 uppercase tracking-wider">{{ $label }}</span>
                    ...
                    <div class="space-y-2 pt-2 border-t border-slate-100">
                        ...
                        <p class="text-xs sm:text-sm text-slate-600 leading-relaxed whitespace-pre-line font-normal">{{ $estate->description }}</p>
                    </div>
                </div>
            ...
            <div class="lg:col-span-4 lg:sticky lg:top-22">
                <div class="hidden lg:block bg-white p-6 rounded-xl border border-slate-200/80 shadow-sm space-y-5">
                    <div class="pb-4 border-b border-slate-100 flex items-center gap-3">
                        <div class="w-12 h-12 rounded-full bg-blue-50 text-blue-600 font-bold flex items-center justify-center shrink-0 border border-slate-100">
                            {{ substr($agent->display_brand_name, 0, 2) }}
                        </div>
                        <div>
                            <span class="text-[10px] font-bold text-slate-400 uppercase tracking-wider block">Dikelola Oleh</span>
    ...
    {{-- Mobile Floating Bottom WA CTA Bar --}}
    @if ($defaultWa)
        <div class="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-slate-200/80 p-3">
