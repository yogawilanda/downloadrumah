...
| @usage    : Compact Grid Partial View for User Register Form
...
<div x-show="mode === 'register'" x-cloak x-transition:enter="transition ease-out duration-200"
    ...
    <form wire:submit="register" class="space-y-3">
        <!-- ROW 1: Nama Lengkap & Nomor Telepon (2 Kolom) -->
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div>
                <label class="block text-xs font-semibold text-gray-700 mb-1">Nomor Telepon <span class="text-red-500">*</span></label>
        ...
        <!-- ROW 3: Password & Konfirmasi Password (2 Kolom) -->
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div>
                ...
                <div class="relative">
                    ...
                    <button type="button" @click="showPassword = !showPassword"
                        ...
                        tabindex="-1">
                        <svg x-show="!showPassword" class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
                            <path d="M2 12s3-7 10-7 10 7 10 7-3 7-10 7-10-7-10-7Z" />
                            <circle cx="12" cy="12" r="3" />
                        </svg>
                        <svg x-show="showPassword" x-cloak class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
        ...
        <!-- Real-time Kriteria Password (Compact) -->
        <div class="p-2.5 bg-gray-50 rounded-md text-xs space-y-1 text-gray-600 border border-gray-100">
            <p class="font-medium text-gray-700">Kriteria kata sandi:</p>
            <div class="grid grid-cols-2 gap-1">
                <span class="{{ strlen($registerForm->password) >= 8 ? 'text-emerald-600 font-medium' : '' }}">• Min. 8 Karakter {{ strlen($registerForm->password) >= 8 ? '✓' : '' }}</span>
                <span class="{{ preg_match('/[A-Z]/', $registerForm->password) && preg_match('/[a-z]/', $registerForm->password) ? 'text-emerald-600 font-medium' : '' }}">• Huruf Besar & Kecil {{ preg_match('/[A-Z]/', $registerForm->password) && preg_match('/[a-z]/', $registerForm->password) ? '✓' : '' }}</span>
                <span class="{{ preg_match('/[0-9]/', $registerForm->password) ? 'text-emerald-600 font-medium' : '' }}">• Mengandung Angka {{ preg_match('/[0-9]/', $registerForm->password) ? '✓' : '' }}</span>
                <span class="{{ preg_match('/[\W_]/', $registerForm->password) ? 'text-emerald-600 font-medium' : '' }}">• Simbol (@$!%*#?&) {{ preg_match('/[\W_]/', $registerForm->password) ? '✓' : '' }}</span>
            </div>
        </div>

        <!-- Feedback Password & Error Messages -->
        @if (strlen($registerForm->password_confirmation) > 0)
            <p class="text-xs {{ $registerForm->password === $registerForm->password_confirmation ? 'text-emerald-600 font-medium' : 'text-red-500' }}">
                {{ $registerForm->password === $registerForm->password_confirmation ? '✓ Kata sandi cocok' : '✗ Kata sandi belum cocok' }}
            </p>
        @endif
        <x-input-error :messages="$errors->get('registerForm.password')" class="text-xs" />
        <x-input-error :messages="$errors->get('registerForm.password_confirmation')" class="text-xs" />
